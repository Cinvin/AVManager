...
def spider_sitemap():
    xmlurl = f"https://www.1pondo.tv/static-seo/sitemap-videos.xml"
    ...
    for urltag in urltags:
        ...
        findcode=re.findall('https://www.1pondo.tv/movies/(.*?)/',loc)
        if len(findcode)==0:
            continue
        code=findcode[0]
        if DBHelper.get_movie_obj(code,'一本道'):
            continue
        ...
        DBHelper.save_movie(code=code, category=2,cid=None, title=title, length=length, rdate=rdate,
                            director=None, studio='一本道', label=None, series=None,
                            piccode=videog, piccount=0, source=1005,
