...
def spider_avlist_by_date():
    listurl=f'https://www.moodyz.com/works/date/'
    ...
    for link in links:
        ...
        print(f'rdate:{rdate}')
        #https://www.moodyz.com/works/list/date/20020301/
        htmlml=CrawlerHelper.get_requests('https://www.moodyz.com'+movielisturl).text
        bsml = BeautifulSoup(htmlml, "html.parser")
        movielinks = bsml.find_all('a', href=re.compile('/works/detail/(.*?)/'))
        for ml in movielinks:
            ...
            av=DBHelper.check_dvdid_exist_with_studioid(dvdid2, studio='ムーディーズ')
            if av:
                av.rdate=rdate
...
def crawler_detail(urlid,rdate):
    url=f'https://www.moodyz.com/works/detail/{urlid}/'
    ...
    for dmmlink in dmmlinks:
        ...
        if dmmurl.startswith('http://www.dmm.co.jp/digital/videoa/-/detail/=/cid='):
            ...
            if DBHelper.check_dvdid_exist_with_studioid(code, studio='ムーディーズ'):
                return
...
if __name__=='__main__':
    ...
    #crawler_detail('mdy030','2006-01-13')
