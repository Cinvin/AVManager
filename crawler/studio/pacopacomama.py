...
def spider_sitemap():
    xmlurl = f"https://www.pacopacomama.com/static-seo/sitemap-videos.xml"
    ...
    for urltag in urltags:
        ...
        findcode=re.findall('https://www.pacopacomama.com/movies/(.*?)/',loc)
        if len(findcode)==0:
            continue
        code=findcode[0]
        if DBHelper.get_movie_obj(code,'パコパコママ'):
            continue
        ...
        DBHelper.save_movie(code=code, category=2,cid=None, title=title, length=length, rdate=rdate,
                            director=None, studio='パコパコママ', label=None, series=None,
                            piccode=videog, piccount=0, source=1008,
