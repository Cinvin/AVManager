...
def spider_sitemap():
    xmlurl = f"https://www.10musume.com/static-seo/sitemap-videos.xml"
    ...
    for urltag in urltags:
        ...
        findcode=re.findall('http://www.10musume.com/movies/(.*?)/',loc)
        if len(findcode)==0:
            continue
        code=findcode[0]
        if DBHelper.get_movie_obj(code,'10musume'):
            continue
        ...
        DBHelper.save_movie(code=code, category=2,cid=None, title=title, length=length, rdate=rdate,
                            director=None, studio='10musume', label=None, series=None,
                            piccode=videog, piccount=0, source=1007,
